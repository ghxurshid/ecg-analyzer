...
public class VitalSignsMonitorShowcaseFragment extends ShowcaseExampleBaseFragment {
    ...
    protected void initExample() {
        final DataProviderBase<VitalSignsData> dataProvider = new BluetoothVitalSignsDataProvider(getActivity());
        //final DataProviderBase<VitalSignsData> dataProvider = new DefaultVitalSignsDataProvider(getActivity());

        setUpChart(dataProvider);

        dataProvider.getData().buffer(40, TimeUnit.MILLISECONDS).doOnNext(ecgData -> {
    ...
    private void setUpChart(DataProviderBase<VitalSignsData> dataProvider) {
